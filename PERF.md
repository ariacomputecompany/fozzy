...
- ✅ Stop scenario fuzz from invoking full `run`/`explore` pipelines per input.
...
- ✅ Make artifact generation lazy/opt-in on hot execution paths.
...
- ✅ Remove duplicated run-manifest writes in the same command path.
...
- ✅ Avoid retaining full `ScenarioRun` payloads in `test` when not needed.
...
- ✅ Rework shrink loop to avoid clone-heavy delta-debugging.
...
- ✅ Remove full `ExecCtx` cloning in `assert_throws` / `assert_rejects`.
   - `exec_expect_failure` clones complete runtime state, including maps/queues/memory/events.
   - Ref: `src/runtime/engine.rs:3253`

- ✅ Optimize snapshot/restore model (`fs_snapshot`, host fs snapshot/restore).
...
- ✅ Rework network queue/inbox structures for large message volume.
...
- ✅ Make profile bundle loading incremental/cached.
...
- ✅ Avoid repeated full profile recomputation in metric checks.
...
- ✅ Split `profile doctor` into fast vs deep modes.
...
- ✅ Avoid full run-history scans for aliases (`latest`, `last-pass`, `last-fail`).
...
- ✅ Bound large proc/http trace payload fields.
