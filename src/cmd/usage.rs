    ...
    UsageDoc {
        ...
        items: vec![
            ...
            UsageItem {
                ...
                how: "fozzy report show <runId|trace> --format junit; fozzy report query <runId> --path '.findings[].title'; fozzy report flaky <run1> <run2> --flake-budget 5. Aliases (`latest`, `last-pass`, `last-fail`) are supported, but CI should prefer explicit run ids or trace paths when race-sensitive.".to_string(),
            },
            UsageItem {
            ...
            UsageItem {
                ...
                how: "fozzy run tests/example.fozzy.json --det --profile-capture full --record ./trace.fozzy --json; fozzy profile env --json; fozzy profile top <runId|trace> --heap --latency --io --sched --limit 20; fozzy profile diff <left> <right> --heap --latency --json; fozzy profile explain <runId|trace> --diff-with <baseline>; fozzy profile export <runId|trace> --format otlp --out profile.otlp.json; fozzy profile shrink <runId|trace> --metric p99_latency --direction increase --minimize all (returns status=no_feasible_shrink_found instead of hard error when contract preservation is impossible and emits dedicated shrink profile artifacts under `artifactsDir` instead of mutating the source run directory); fozzy profile doctor <runId|trace> --json. CPU profile commands are only valid for traces that already contain real `sample` events; span-derived pseudo-CPU profiles are not supported. Strictest setting: strict mode is on by default; add `--unsafe` only to opt out.".to_string(),
            },
            UsageItem {
